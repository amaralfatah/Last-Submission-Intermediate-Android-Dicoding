...
import com.bangkit.intermediateandroid.submission1intermediate2.databinding.ActivityMapsBinding
...
//        val bearertoken = "Bearer $token"
        CoroutineScope(Dispatchers.IO).launch {
            val response = service.getAllStoriesWithLocation("Bearer $token",1)
                .await()
            ...
            if (response.error == true) {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    Toast.makeText(this@MapsActivity, "DATA DARI MAPS " + listStory?.map { it.name }?.count(), Toast.LENGTH_LONG).show()
                    Log.d("DATA DARI MAPS ", listStory?.map { it.name }.toString())

                    listStory?.map { user ->
                        val latlng = LatLng(user.lat as Double, user.lon as Double)
...
//        addManyMarker()
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.optionmenu_maps, menu)
        return true
    }
